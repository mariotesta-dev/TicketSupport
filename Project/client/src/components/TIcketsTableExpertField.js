...
function TicketsTableExpertField({ticket}) {
    ...
    const [selectedExpert, setSelectedExpert] = useState(ticket.assignedTo != null ? ({label: ticket.assignedTo.name + " " + ticket.assignedTo.surname, value: ticket.assignedTo}) : null);
    const [priority, setPriority] = useState(ticket.priority ? ({label: ticket.priority, value: ticket.priority}) : null);
    ...
    useEffect(() => {
      const handleGetExperts = async () => {
        try {
          ...
          setExperts(res.map((expert) => ({ label: expert.name + " " + expert.surname, value: expert })));
        } catch (error) {
    ...
    const assignExpert = () => {
        try {
            let res = managersAPI.assignTicketToExpert(ticket.id, selectedExpert.id, priority);
    ...
    return (
      <>
        <Button variant={"outline"} fontWeight={"normal"} onClick={onOpen}>
			      {ticket.assignedTo != null ? (ticket.assignedTo.name + " " + ticket.assignedTo.surname) : "unassigned"}
		</Button>
